...
export const useAppStore = create<AppState>()((set, get) => ({
  ...
  async openFileBuffer(buffer, fileName) {
    ...
    // Strategy: avoid __rawBuffer entirely for parent-sent buffers.
    // Instead, always use the blob URL path in PDFViewer.
    // Blob URL is immune to ArrayBuffer detachment — it's a browser-managed URL.
    // Steps:
    //   1. Create Blob from the incoming buffer (bytes copied into Blob storage)
    //   2. Create File from the Blob
    //   3. Do NOT attach __rawBuffer — PDFViewer will use URL.createObjectURL(file)
    //      which reads from the Blob storage (always valid, never detached)
    const blob = new Blob([buffer], { type: mime });
    const file = new File([blob], fileName, { type: mime });
    // Deliberately NOT attaching __rawBuffer — forces PDFViewer to use the
    // blob URL path which is immune to ArrayBuffer detachment issues.
    // The Blob stores the bytes independently of the original ArrayBuffer.
