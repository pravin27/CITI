...
export default function App() {
  ...
  // Expose confirm popup opener globally so ThumbnailSidebar/MultiDocSidebar
  // can trigger the full-viewer overlay without prop drilling
  React.useEffect(() => {
    (window as any).__doccapture_openRetriggerConfirm = () => setShowConfirm(true);
    return () => { delete (window as any).__doccapture_openRetriggerConfirm; };
  }, []);
  ...
  React.useEffect(() => {
    (window as any).__doccapture_openSplitMerge = () => {
      ...
      // ── Patch payload with CURRENT store state before sending to SM ──────
      // When SM has reclassified and sent SM_RECLASSIFY, the store holds
      // the updated categories/documents. We must send THOSE to SM on the
      // next open — not the stale original from the parent message.
      const storeCategories = useAppStore.getState().categories;
      const patchedPayload: Record<string, unknown> = { ...originalMsg };

      // LOAD_SINGLEDOC: patch categories[]
      if (storeCategories?.length) {
        patchedPayload.categories = storeCategories;
      }

      // LOAD_MANIFEST: patch manifest.documents with current state.
      // Convert internal `categoryId` field back to `category` string
      // so SM app receives the same format it originally sent.
      const mds = multiDocRef.current?.state;
      if (mds?.documents?.length && patchedPayload.manifest) {
        const docsForSM = mds.documents.map((d: any) => ({
          ...d,
          category:   d.categoryId,   // SM expects "category" not "categoryId"
          categoryId: undefined,       // remove internal field
        }));
        patchedPayload.manifest = {
          ...(patchedPayload.manifest as Record<string, unknown>),
          documents: docsForSM,
        };
      }
      ...
      const smMessage: Record<string, unknown> = {
        ...
        payload: patchedPayload,             // patched with current state
        viewerState: {
  ...
  // ── multiDocRef: lets the message handler (registered once) always access
  // the latest multiDoc state without being stale from the [] closure ────────
  const multiDocRef = React.useRef<ReturnType<typeof useMultiDoc> | null>(null);
  ...
  React.useEffect(() => {
    const handler = (event: MessageEvent) => {
      ...
      // SM_RECLASSIFY — SM iframe sends reclassified data back to viewer.
      // Supports all three load scenarios:
      //   LOAD_SINGLEDOC      → categories[] changed
      //   MultiDoc:SinglePage → documents[].category changed
      //   MultiDoc:MultiPage  → documents[].pageCategories changed
      if (data.type === 'SM_RECLASSIFY') {
        // Always close SM and show viewer regardless of whether data changed
        (window as any).__doccapture_sm_active = false;
        setSmActive(false);

        // Re-apply isSplitScreen from SM payload — ensures bar shows even if
        // the store value was false (e.g. store was reset by a new doc load)
        if (data.isSplitScreen === true) {
        ...
        if (!hasCategories && !hasDocuments) {
          // No data sent — treat same as SPLIT_MERGE_EXIT (no change in SM app)
          if (isEmbedded) window.parent.postMessage({ type: 'SPLIT_MERGE_EXITED', reason: 'no_change' }, '*');
          return;
        }

        // Option A: Viewer owns the original snapshot — SM sends updated data only.
        const PALETTE = ['blue','teal','amber','gray','purple','coral','green','red'];
        const st = useAppStore.getState();

        // ── LOAD_SINGLEDOC: updated flat categories from SM ───────────────
        if (hasCategories) {
          const updated = (data.categories as any[]).map((c: any, i: number) => ({
            ...c, color: c.color ?? PALETTE[i % PALETTE.length],
          }));
          st.setCategories(updated as any);
          st.setHasReclassification(true);
        }

        // ── MultiDoc: updated documents from SM ───────────────────────────
        const md = multiDocRef.current;
        if (hasDocuments && md?.state) {
          md.updateDocuments?.(data.documents as any[]);
          st.setHasReclassification(true);
        }

        // Reset retrigger bar to 'idle' so it reappears with fresh data.
        // The sidebar keeps retriggerState in local component state — it persists
        // as 'done' from any prior Retrigger click since the component stays mounted.
        // Calling this reset makes the bar visible again after SM_RECLASSIFY.
        (window as any).__doccapture_resetRetriggerBar?.();

        // NOTE: st.originalCategories is intentionally NOT touched here.
        return;
      }
  ...
  // Keep ref in sync so message handler always has latest state
  multiDocRef.current = multiDoc;

  // Expose updateDocuments globally so MultiDocSidebar handleDiscard can call it
  React.useEffect(() => {
    (window as any).__doccapture_updateDocuments = (docs: any[]) => {
      multiDoc.updateDocuments?.(docs);
    };
    return () => { delete (window as any).__doccapture_updateDocuments; };
  }, [multiDoc]);
  ...
  const applyDocMeta = useCallback((
    ...
    waitForAdapterReady().then(() => {
      ...
      if (meta.categories) {
        ...
        // Snapshot: store as originalCategories on EVERY fresh parent load.
        // This overwrites any previous snapshot so if parent pushes a new
        // LOAD_SINGLEDOC the fresh categories become the new "original".
        // SM_RECLASSIFY does NOT touch this — so Discard always reverts
        // to the last PARENT-provided state, not an SM-modified state.
        useAppStore.getState().setOriginalCategories(withColors as any);
        useAppStore.getState().setHasReclassification(false);
        useAppStore.getState().setOriginalDocuments(null);  // reset MultiDoc snapshot
      }
      ...
      if (meta.captures && meta.captures.length > 0) {
        import('./utils/coords').then(({ extractRawCoords, normaliseCoords, normaliseBatch }) => {
          ...
          const coordSpace2 = (meta as any)?.coordinateSpace as 'px'|'pt'|'norm'|undefined;
          const rawCapsHinted = coordSpace2
            ? rawCaps.map((c: any) => ({ ...c, _rawUnit: coordSpace2 }))
            : rawCaps;
  ...
  const bridge = useEventBridge({
    ...
    onLoadSingleDoc: (buffer, fileName, meta) => {
      ...
      if ((meta as any)?.isSplitScreen !== undefined) {
        const val = !!(meta as any).isSplitScreen;
        console.log('[App onLoadSingleDoc] isSplitScreen=', val, 'meta=', JSON.stringify(meta).slice(0,200));
        useAppStore.getState().setIsSplitScreen(val);
        if (val) useAppStore.getState().setSidebarOpen(true);
        console.log('[App] after set: isSplitScreen=', useAppStore.getState().isSplitScreen, 'sidebarOpen=', useAppStore.getState().sidebarOpen);
      } else {
        console.log('[App onLoadSingleDoc] isSplitScreen NOT in meta. meta keys=', Object.keys(meta ?? {}));
      }
      if ((meta as any)?.openThumbnail)
        useAppStore.getState().setSidebarOpen(true);
    },

    onLoadManifest: (manifest, activeDocId, activeBuffer, meta) => {
      // Pre-seed the file cache with activeBuffer BEFORE loadManifest triggers
      // the auto-select (which calls selectDoc via setTimeout). Without this,
      // selectDoc sees no cache hit and fires DOC_REQUEST even though the buffer
      // is already available in the initial LOAD_MANIFEST payload.
      if (activeBuffer && activeBuffer.byteLength > 0 && activeDocId) {
        ...
        const blob = new Blob([activeBuffer], { type: mime });
        const file = new File([blob], name, { type: mime });
        try {
          Object.defineProperty(file, '__docId',
            { value: activeDocId, enumerable: false, writable: false });
        } catch (_) {}
        // Inject directly into multiDoc file cache before loadManifest auto-select fires
        const blobUrl = URL.createObjectURL(blob);
        multiDoc.preSeedCache?.(activeDocId, file, blobUrl);
      }
      ...
      if ((meta as any)?.isSplitScreen !== undefined) {
        const val = !!(meta as any).isSplitScreen;
        console.log('[App onLoadManifest] isSplitScreen=', val, 'meta=', JSON.stringify(meta).slice(0,200));
        useAppStore.getState().setIsSplitScreen(val);
        if (val) useAppStore.getState().setSidebarOpen(true);
        console.log('[App] after set: isSplitScreen=', useAppStore.getState().isSplitScreen, 'sidebarOpen=', useAppStore.getState().sidebarOpen);
      } else {
        console.log('[App onLoadManifest] isSplitScreen NOT in meta. meta keys=', Object.keys(meta ?? {}));
      }
      if ((meta as any)?.openThumbnail)
        useAppStore.getState().setSidebarOpen(true);

      // Snapshot manifest documents for MultiDoc Discard/Reset
      if (manifest.documents?.length) {
        useAppStore.getState().setOriginalDocuments(
          (manifest.documents as any[]).map((d: any) => ({ ...d }))
        );
  ...
  // ── VIEWER_STATE_CHANGED — single unified event for parent ─────────────────
  // Fires whenever the user navigates to a different page OR switches document.
  // Covers: toolbar nav, thumbnail click, PDF scroll, keyboard, MultiDoc doc switch.
  // Parent subscribes to one event type for all navigation notifications.
  const prevStateRef = React.useRef<{ page: number; docId: string | null }>({
    page: 0, docId: null,
  });
  React.useEffect(() => {
    if (!isEmbedded) return;
  ...
  const handleConfirmProceed = React.useCallback(() => {
    ...
    // Trigger success state in sidebar (shows "Successfully sent" then hides bar)
  ...
  return (
    <div className="flex flex-col h-full bg-[#060d1a] overflow-hidden">
      ...
      {/* ── Retrigger confirmation popup — full viewer overlay ───────────── */}
      {showConfirm && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <i className="ti ti-arrows-transfer-up" aria-hidden="true"
                style={{ fontSize: 20, color: '#374151', lineHeight: 1 }} />
            </div>
            {/* Title */}
            <span style={{
            ...
            <div style={{ display: 'flex', gap: 8, width: '100%', marginTop: 4 }}>
              ...
              <button onClick={handleConfirmProceed} style={{
                flex: 1, padding: '7px 0', borderRadius: '7px',
