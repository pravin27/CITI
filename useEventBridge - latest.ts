    ...
    const handler = (event: MessageEvent) => {
      ...
      switch (type) {
        ...
        case 'LOAD_SINGLEDOC': {
          ...
          if (!buffer || !(buffer instanceof ArrayBuffer) || (buffer as ArrayBuffer).byteLength === 0) {
            postToParent({
              type: 'DOC_LOAD_ERROR', code: 'INVALID_BUFFER',
              reason: !buffer
                ? 'LOAD_SINGLEDOC received no buffer — ensure buffer is included in the message.'
                : (buffer as ArrayBuffer).byteLength === 0
                ? 'LOAD_SINGLEDOC received an empty ArrayBuffer (byteLength=0). The buffer may have been transferred already.'
                : 'LOAD_SINGLEDOC buffer is not an ArrayBuffer.',
              fileName,
            });
            return;
          }
        ...
        case 'DOC_RESPONSE': {
          ...
          // Validate buffer before resolving
          if (!buffer || !(buffer instanceof ArrayBuffer) || (buffer as ArrayBuffer).byteLength === 0) {
            clearTimeout(pending.timer);
            pendingDocs.current.delete(docId);
            const reason = !buffer
              ? `DOC_RESPONSE for "${docId}" contained no buffer.`
              : `DOC_RESPONSE for "${docId}" had an empty ArrayBuffer (byteLength=0). The buffer may have been transferred already.`;
