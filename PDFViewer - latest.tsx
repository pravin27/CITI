    ...
    if (ds.wordIndex.size > 0) {
      ...
      for (const [pg, entries] of ds.wordIndex) {
        ...
        for (const e of entries) {
          // Exact lowercase match — "invoice." ≠ "invoice" per user requirement
          const wt = e.text.toLowerCase().trim(); if (!watch.has(wt)) continue;
...
function paintPage(ctx: Ctx2D, W: number, H: number, page: number,
  ...
  if (hasWI && pl3?.size) {
    ...
    for (const span of spans) {
      // Exact lowercase — no punct stripping, matches user expectation
      ...
      if (pdfjsDoc) {
        ...
        pdfjsDoc.getPage(pageNum).then((pdfjsPage: any) => {
          ...
          for (const item of tc.items) {
            ...
            if (!watch.has(wt)) continue;

            // pdfjs transform: [a, b, c, d, tx, ty] — tx/ty in PDF pts, origin bottom-left
            const tx = item.transform?.[4] ?? 0;
            const ty = item.transform?.[5] ?? 0;
            const fw = (item.width  ?? 0) / pw;
            const fh = Math.abs(item.transform?.[3] ?? item.height ?? 10) / ph;
            const fx = tx / pw;
            const fy = 1 - (ty / ph) - fh;   // flip Y: PDF origin is bottom-left

            if (fx < 0 || fy < 0 || fw <= 0 || fh <= 0) continue;
            if (!pm.has(wt)) pm.set(wt, []);
            pm.get(wt)!.push({ x: fx, y: fy, w: fw, h: fh });
          }

          if (pm.size) {
            // Extend l3c with this page's entries — don't replace existing entries
            cachesRef.current.l3c.set(pageNum, pm);
            // Trigger a repaint now that l3c has new data
