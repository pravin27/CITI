...
const PAD_TOP  = 0.08;  // 8%  of h added above text
const PAD_BOT  = 0.05;  // 5%  of h added below text (less — baseline is near bottom)
const PAD_W    = 0.10;  // 10% of h added left + right
...
export function PDFViewer({
  ...
  function pageStateKey(pageNum: number, W: number, H: number, ds: DrawState): string {
    ...
    return `${W}x${H}|${ds.activeFieldId ?? ''}|${rects?.length ?? 0}|${r0Key}|${prKey}|${ds.search.currentIndex}`;
  }
      ...
      if (pdfjsDoc) {
        ...
        pdfjsDoc.getPage(pageNum).then((pdfjsPage: any) => {
          ...
          for (const item of tc.items) {
            ...
            const fh  = Math.abs(item.transform?.[3] ?? item.height ?? 10) / ph;
  ...
  function updatePulse() {
    ...
    // vp.width/height include devicePixelRatio but pv.div is CSS-sized.
    // Divide by DPR so pulse CSS px aligns exactly with the canvas overlay.
    // Use pv.div actual rendered CSS size — matches the canvas overlay exactly.
    // canvas has style="width:100%;height:100%" so it stretches to clientWidth/Height.
    // Using clientWidth avoids floor/round rounding drift vs raw vp.width.
    const cW = pv.div.clientWidth  || W;
    const cH = pv.div.clientHeight || H;
