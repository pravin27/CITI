...
import { PALETTE_MAP, PALETTE_BG_OPACITY } from '../adapters/types';
...
function resolveCatColor(color: string): {
  ...
  if (color in PALETTE_MAP) {
    ...
    return {
      bg:        p.bg,       // rgba at PALETTE_BG_OPACITY
  ...
  // Hex color (from categoryColor hash or parent) — light pastel bg + vivid border
  if (color.startsWith('#') && color.length >= 7) {
    const r = parseInt(color.slice(1, 3), 16);
    const g = parseInt(color.slice(3, 5), 16);
    const b = parseInt(color.slice(5, 7), 16);
    return {
      bg:        `rgba(${r},${g},${b},${PALETTE_BG_OPACITY})`,
      border:    color,
      text:      '#1a1a1a',
      badgeBg:   color,
      badgeText: '#ffffff',
...
function PageThumbRow({ pageNum, isActive, cat, onClick, adapterKey, isDocActive }: {
  pageNum: number;
  isActive: boolean;
  ...
  const [pdfDocVersion, setPdfDocVersion] = useState(
    () => (window as any).__doccapture_pdfDocVersion ?? 0
  );
  ...
  useEffect(() => {
    // Use a Set so ALL visible PageThumbRow instances get notified when PDF loads
    if (!(window as any).__doccapture_pdfDocListeners) {
      (window as any).__doccapture_pdfDocListeners = new Set<()=>void>();
    }
    const onReady = () => setPdfDocVersion((v: number) => v + 1);
    ((window as any).__doccapture_pdfDocListeners as Set<()=>void>).add(onReady);
    return () => {
      ((window as any).__doccapture_pdfDocListeners as Set<()=>void>)?.delete(onReady);
    };
  }, []);

  useEffect(() => {
    if (!adapterKey || !isDocActive) return;  // don't render for inactive docs
  ...
  }, [pageNum, adapterKey, paint, pdfDocVersion, isDocActive]);
  ...
  return (
    <div
      ...
      style={{
        ...
        width:         '100%',
        padding:       '4px 8px',   // matches SingleDoc ThumbnailItem
...
export function MultiDocSidebar({ state, isLoading, loadError, onSelectDoc, onSelectPage, onClose }: MultiDocSidebarProps) {
  ...
  const [docSwitchKey, setDocSwitchKey] = useState(0);
  ...
  // Include activeDocId in adapterKey so cache is per-document
  // This ensures switching docs invalidates ALL cached thumbnails for old doc
  const adapterKey  = adAdapter && adFileName
    ? `${activeDocId ?? ''}:${adAdapter.constructor.name}:${adFileName}:${adFileSize}:${adPageCount}`
    : '';
  ...
  useEffect(() => {
    ...
    // Clear all PageThumbRow canvases on doc switch — forces fresh load
    (window as any).__doccapture_clearThumbs = () => {
      // Clear existing listener Set so remounted rows start fresh
      (window as any).__doccapture_pdfDocListeners = new Set();
      setDocSwitchKey(k => k + 1);
    };
                  ...
                  <div key={OTHERS_KEY}>
                    ...
                    {!collapsedCats.has(key) && pages.map(pg => (
                      <PageThumbRow
                        ...
                        onClick={() => onSelectPage(pg)}
                        adapterKey={adapterKey}
                        isDocActive={activeDoc?.id === activeDocId}
                      />
                    ))}
                  </div>
                );
