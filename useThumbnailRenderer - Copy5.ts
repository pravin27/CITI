...
export function useThumbnailRenderer(adapter: AdapterInstance | null) {
  ...
  const doRender = useCallback(async (page: number): Promise<ImageBitmap | null> => {
    ...
    if (ad.constructor.name === 'PDFAdapter') {
      ...
      if (!doc) {
        console.warn('[thumb] __tovPdfDoc not available for page', page);
        return null;
      }
      ...
      console.log('[thumb] rendered page', page, 'size:', result.width, 'x', result.height);
      return result;
    }
  ...
  const processOne = useCallback(async () => {
    ...
    try {
      ...
      // Always log — helps diagnose ECS/deployment issues
      console.warn('[thumb] render failed p=' + page + ':', err);
    } finally {
      inFlight.current = false;
    }
  ...
  useEffect(() => {
    ...
    console.log('[thumb] sweep effect runs, pc=', pc, 'BG_MAX=', BG_MAX_PAGES);
    if (pc === 0 || pc > BG_MAX_PAGES) { console.warn('[thumb] sweep skipped: pc=', pc); return; }
    ...
    const go = () => {
      ...
      console.log('[thumb] onReady fired, pageCount=', total);
      if (total === 0) { console.warn('[thumb] pageCount=0, sweep aborted'); return; }
      let p = 1;
      const step = () => {
        if (cancelled) return;
        const tot = pageCount || (adapter.pageCount ?? 0);
        console.log('[thumb] sweep step p=', p, 'of', tot, 'queued enqLow');
