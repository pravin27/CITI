...
// Rendered thumbnail width in pixels. Reduce to show smaller previews.
// THUMB_BASE_H in ThumbnailSidebar.tsx controls the display height slot.
const THUMB_W           = 72;  // was 96 — reduced for compact thumbnails
const PRELOAD_AHEAD     = 2;
const BG_MAX_PAGES      = 1000;
const INTERACTION_PAUSE = 1200;  // ms to pause bg renders after any interaction
const RENDER_BATCH      = 3;     // pages to render per scheduler tick (reduces setTimeout overhead)
const SCHEDULER_DELAY   = 16;    // ms between batches (~1 frame) — yields to navigation
...
function getAdapterKey(adapter: AdapterInstance | null): string {
  ...
  const adName = adapter instanceof PDFAdapter ? 'PDF' : adapter instanceof ImageAdapter ? 'Img' : 'Other';
  return `${adName}:${s.fileName}:${s.file?.size ?? 0}:${adapter.pageCount ?? 0}`;
}
...
export function useThumbnailRenderer(adapter: AdapterInstance | null) {
  ...
  const doRender = useCallback(async (page: number): Promise<ImageBitmap | null> => {
    ...
    // Snapshot the adapter key at render-start.
    // If the key changes mid-await (new doc loaded), we discard the result
    // so we never paint a thumb from the wrong document.
    const renderKey = adapterKey.current;
    const isStillCurrent = () => adapterKey.current === renderKey && adapterRef.current === ad;

    try {
      if (ad instanceof PDFAdapter) {
        const thumbDoc = (window as any).__tovThumbDoc ?? (window as any).__tovPdfDoc;
        if (!thumbDoc) return null;
        const pdfPage = await thumbDoc.getPage(page);
        if (!isStillCurrent()) { try { pdfPage.cleanup(); } catch (_) {} return null; }
      ...
      if (ad instanceof ImageAdapter) {
        const frame = await (ad as ImageAdapter).getFrameAsync(page);
        if (!isStillCurrent()) return null;
        if (!frame) return null;
        // Guard: bitmap may have been closed by dispose() between getFrameAsync and drawImage
        if (!frame.bitmap || (frame.bitmap as any).width === 0) return null;
      ...
      // Spreadsheet / DocAdapter
      if (!isStillCurrent()) return null;
      const src = ad.getPageCanvas?.(page);
      if (!src || src.width === 0 || src.height === 0) return null; // not ready yet
      ...
      // Any crash (disposed bitmap, destroyed pdfjs page, etc.) → return null
      // The thumbnail will be retried when the page scrolls back into view.
      if (!String(err).includes('canvas-not-ready'))
        console.warn('[useThumbnailRenderer] doRender page', page, '→', String(err).split('')[0]);
      return null;
    }
  }, []); // adapterRef and adapterKey are stable refs — no deps needed
  ...
  const scheduleNext = useCallback(() => {
    ...
    }, SCHEDULER_DELAY);  // yield one frame between batches
  }, []);

  const kick = useCallback(() => {
    if (running.current && timerRef.current !== null) return;
    if (running.current && inFlight.current) return;
    running.current = true;
    scheduleNext();
  }, [scheduleNext]);

  // Expose a way to pause bg rendering during navigation
  // Called by navigateToPage in PDFViewer so pdfjs gets uncontested main thread time
  ...
  const processOne = useCallback(async () => {
    ...
    // Render the page
    const renderKey = adapterKey.current;  // snapshot before async
    inFlight.current = true;
    try {
      const bm = await doRender(page);
      // doRender already returns null when adapter changed, but double-guard here
      // so we never paint a bitmap onto a canvas that now belongs to a different doc.
      if (bm && adapterKey.current === renderKey) {
        storeBitmap(page, bm);
        const cv = canvasMap.current.get(page);
        if (cv) { cv.width = bm.width; cv.height = bm.height; cv.getContext('2d')?.drawImage(bm, 0, 0); }
      } else if (bm) {
        // Adapter changed — close the bitmap we just created to free GPU memory
  ...
  const registerThumb = useCallback((
    ...
    // When visiblePages is empty (initial load or category switch), enqueue
    // all registrations as high-priority so all thumbnails render promptly.
    const vp = [...visiblePages.current];
    if (vp.length === 0) {
      enqHigh(page); kick(); return;
    }
    const lo = Math.min(...vp) - 2;
    const hi = Math.max(...vp) + 2;
  ...
  useEffect(() => {
    ...
    // Update adapterKey BEFORE clearQueues so any in-flight doRender
    // sees the new key via isStillCurrent() and discards its stale result.
